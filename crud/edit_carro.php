...
// Inicializa variáveis para armazenar os dados do carro
$id_carro = $_GET['id'];
$modelo = '';
$ano = '';
$placa = '';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt_update->bindParam(':ano', $ano, PDO::PARAM_INT);
        $stmt_update->bindParam(':placa', $placa, PDO::PARAM_STR);
        $stmt_update->bindParam(':cliente_id', $cliente_id, PDO::PARAM_INT);
        $stmt_update->bindParam(':id', $id_carro, PDO::PARAM_INT);

        $stmt_update->execute();

        // Redirecionar para uma página de sucesso ou para a listagem de carros após a atualização
        echo "<script>window.location='../account-bookings.php?carro_pagina=1#tab-2';</script>";
        exit();
    }

    // Query para selecionar os dados do carro a ser editado
    $sql_select = "SELECT * FROM carros WHERE id = :id";
    $stmt_select = $conn->prepare($sql_select);
    $stmt_select->bindParam(':id', $id_carro, PDO::PARAM_INT);
    $stmt_select->execute();

    $carro = $stmt_select->fetch(PDO::FETCH_ASSOC);

    if (!$carro) {
        // Se não encontrar o carro, redirecionar ou exibir mensagem de erro
       echo 'error 404';
        exit();
    }
    ...
    $stmt_clientes = $conn->query($sql_clientes);
        ...
        <section class="pt-4 pt-md-5 pb-0">
            <div class="container">
                <div class="row">
                    ...
                    <h2>Editar Carro - <?php echo htmlspecialchars($modelo); ?></h2>
                    </div>



                 
        <!-- Formulário de edição de carro aqui -->
        <form method="POST">
            <div class="mb-3">
                <label for="modelo">Modelo:</label>
                <input type="text" id="modelo" name="modelo" class="form-control" value="<?php echo htmlspecialchars($modelo); ?>" required>
            </div>
            <div class="mb-3">
                <label for="ano">Ano:</label>
                <input type="number" id="ano" name="ano" class="form-control" value="<?php echo htmlspecialchars($ano); ?>" required>
            </div>
            <div class="mb-3">
                <label for="placa">Placa:</label>
                <input type="text" id="placa" name="placa" class="form-control" value="<?php echo htmlspecialchars($placa); ?>" required>
            </div>
            <div class="mb-3">
                <label for="cliente_id">Cliente:</label>
                <select id="cliente_id" name="cliente_id" class="form-control" required>
                    <?php foreach ($clientes as $cliente) : ?>
                        <option value="<?php echo htmlspecialchars($cliente['id']); ?>" <?php echo ($cliente['id'] == $cliente_id) ? 'selected' : ''; ?>>
